...
function buildSystemPrompt(context: ChatContext, chatConfig: Pick<OpenAIChatConfig, "maxReplyChars"> | null = null): string {
  ...
  return [
    ...
    "Responda em português do Brasil, de forma objetiva, direta ao ponto e focada em identificar números candidatos.",
    `Limite a resposta a no máximo ${chatConfig?.maxReplyChars ?? DEFAULT_MAX_REPLY_CHARS} caracteres, use tópicos curtos e evite texto explicativo longo.`,
    "Comece direto pelos números: liste grupos como Base, Apoio, Ousadia, Evitar/Fracos e uma combinação sugerida compatível com a loteria quando fizer sentido.",
    "Não inclua introduções, conclusões, avisos, alertas, ressalvas sobre riscos, promessas ou explicações genéricas sobre aleatoriedade; o usuário já sabe que é apenas sugestão.",
    "Use somente o contexto de concursos e estatísticas enviado pelo app; se faltar dado, responda em uma frase curta pedindo carregar, filtrar ou ampliar o recorte.",
